...
window.addEventListener('load', function () {
    ...
    var listComponent = Vue.component('items-list-component', {
        ...
        mounted() {
            // console.log('64created', this.items)
            //         setTimeout(() => {
            //             this.items.forEach((item1) => {
            //             console.log(item1)
            //         listComponent.multiSelect(item1);
            //     })
            // },50)
        },
        methods: {
    ...
    var upgradeApp = new Vue({
        ...
        mounted: function mounted() {
            ...
            axios.post('http://boxgo.cc/players/selectPlayerArms', {
                ...
                var data = response.data.body.armsList;
                var all = window.sessionStorage.getItem('allArms')
                if (all) {
                    upgradeApp.upgradable.data = []
                    data.forEach(function(item){
                        upgradeApp.upgradable.data.push(item)
                    })
                } else {
                    upgradeApp.upgradable.data = response.data.body.armsList;
                }
        ...
        methods: {
                ...
                return {
                    ...
                    getFilteredItems: function getFilteredItems(data) {
                        return axios.post('http://boxgo.cc/upgrading/buybox', {
                            ...
                            info: data.info,
                            ziduan: data.ziduan,
                            "pageIndex": _this.forUpgradable.pagination.current,
                            "pageSize":_this.forUpgradable.pagination.countItems
                        });
                    },
            ...
            actionFilterItems: function actionFilterItems(priceMin, priceMax, searchName, quality, priceModifyPercents) {
                console.log(priceMin, priceMax, searchName, quality, priceModifyPercents)
                ...
                this.http().getFilteredItems({
                    ...
                    upgradeApp.forUpgradable.data = response.data.body.list;

                    upgradeApp.forUpgradable.pagination.total = response.data.body.count;
                    console.log(response.data.body.count, 'response.data.body.count')
            })
                ;
            },
